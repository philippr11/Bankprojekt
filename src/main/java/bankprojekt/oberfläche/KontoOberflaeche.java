...
public class KontoOberflaeche extends BorderPane {
	 ...
	 */
	public KontoOberflaeche(Controller controller)
	{
	...
	/**
	 * Bindet den Kontostand an das entsprechende Objekt in der Darstellung, einseitig weil soll nur ausgelesen werden
	 * @param betrag entspricht dem Kontostand
	 */
	public void bindKontostandProperty(ReadOnlyDoubleProperty betrag){
		this.stand.textProperty().bind(betrag.asString());
	}

	/**
	 * Bindet den Boolean ob das Konto gesperrt ist an die Oberfläche, bidirektional
	 * @param gesperrt wahr = gesperrt, false = nicht gesperrt
	 */
	public void bindGesperrtProperty(BooleanProperty gesperrt){
		this.gesperrt.selectedProperty().bindBidirectional(gesperrt);
	}

	/**
	 * Bindet den String der Adresse an die Oberfläche, bidirektional
	 * @param adresse String der Adresse
	 */
	public void bindAdresseProperty(StringProperty adresse){
		this.adresse.textProperty().bindBidirectional(adresse);
	}

	/**
	 * setzt die Kontonummer in der Oberläche
	 * @param kontonummer
	 */
	public void setKontonummer(long kontonummer){
		this.nummer.setText(String.valueOf(kontonummer));
	}

	/**
	 * gibt Nachricht auf View aus
	 * @param meldung Nachricht
	 */
...
/**
 * d) zuerst: ist am einfachsten:
 * Adresse muss als Property verfügbar sein, also brauchen wir
 * getAdresse, setAdresse und adresseProperty
 * get und set haben wir schon, muss nur leicht abgeändert werden, adresseProperty muss neu
 *
 * Kontostand das gleiche, nur setKontostand ist schwierig, Kontostand muss zu ReadonlyDoubleWrapper
 *
 * getter bei istgesperrt isgesperrt, setgesperrt, gesperrtProperty
 *
 * isPositiv bei Kontostand read only, am besten wenn man Kontostand verändert, setKontostand muss Property verändern
 *
 * Controller:
 * neue Klasse, Startanwendung von Javafx muss auf jeden Fall muss von Application erben und startmethode haben
 *
 *Oberfläche:
 * Properties nutzen, setText:				textproperty.bind()
 */
